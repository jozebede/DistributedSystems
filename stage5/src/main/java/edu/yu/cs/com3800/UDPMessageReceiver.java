...
public class UDPMessageReceiver extends Thread implements LoggingServer {
    ...
    private LinkedBlockingQueue<Message> heartBeats;
    private Logger logger;
    private ZooKeeperPeerServer peerServer;

    public UDPMessageReceiver(LinkedBlockingQueue<Message> incomingMessages, LinkedBlockingQueue<Message> heartBeats,InetSocketAddress myAddress, int myPort, ZooKeeperPeerServer peerServer) throws IOException {
        this.incomingMessages = incomingMessages;
        this.heartBeats = heartBeats;
        this.myAddress = myAddress;
        this.myPort = myPort;
        this.logger = initializeLogging("LogFile_"+UDPMessageReceiver.class.getCanonicalName() + "-on-port-" + this.myPort);
        logger.info("Initializing UDP Message Receiver on port " + this.myPort);
        this.setDaemon(true);
    ...
    public void run() {
        ...
        try {
            ...
            logger.info("failed to create receiving socket"+ e);
            return;
        }
        //loop
        while (!this.isInterrupted()) {
            try {
               ...
               // if(received.getMessageType().equals(Message.MessageType.GOSSIP))logger.info("\n \t\t\t GOSSIP RECEIVED\n");
                ...
                if (this.peerServer != null && this.peerServer.isPeerDead(sender)) {
                    logger.info("UDP packet received from dead peer: " + sender.toString() + "; ignoring it.");
                ...
                if (sendLeader(received)) {  // see if we got an Election LOOKING message while we are in FOLLOWING or LEADING?
                    ...
                    if(leader != null){
                       // logger.info(" \n \n sending leader  " + this.peerServer.getCurrentLeader().getProposedLeaderID() + " to server on port " + this.myPort);
                ...
                if(!this.strayElectionMessage(received)){  // if neither sender nor I am looking, and this is an election message, let it disappear
                    //use interrupt-safe version, i.e. offer
                    boolean done = false;
                    while(!done){
                      //  logger.info("Size: " + incomingMessages.size());
                        if (received.getMessageType().equals(Message.MessageType.GOSSIP)){
                            done = this.heartBeats.offer(received); // todo - check this part
                        } else {
                ...
                if (!this.isInterrupted()) {
                    logger.info("Exception caught while trying to receive UDP packet");
    ...
    private void sendElectionReply(byte[] msgContent, InetSocketAddress target) {
        ...
        try (DatagramSocket socket = new DatagramSocket()){
            ...
            logger.info("Election reply sent:\n" + msg.toString());
        }
        catch (IOException e) {
            // logger.info("Failed to send election reply:\n" + msg.toString());
