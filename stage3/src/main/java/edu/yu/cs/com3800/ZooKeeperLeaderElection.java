...
public class ZooKeeperLeaderElection {
    ...
    public ZooKeeperLeaderElection(ZooKeeperPeerServer server, LinkedBlockingQueue<Message> incomingMessages) {
        ...
        initializeLogging();
        logger.info(" init leader:  " + myPeerServer.getServerId());
    ...
    public synchronized Vote lookForLeader() {
        ...
        while (this.myPeerServer.getPeerState() == ZooKeeperPeerServer.ServerState.LOOKING) {
            ...
            //msg =incomingMessages.poll(maxNotificationInterval, TimeUnit.MILLISECONDS);
          ...
          ElectionNotification notification =  getNotificationFromMessage(msg);
                // logger.info(String.valueOf(notification.getProposedLeaderID()));
            ...
            if (notification.getPeerEpoch() < this.proposedEpoch) {
               // a vote whose epoch is < my current epoch  is invalid
           ...
           // logger.info(" STATE  IS " + notification.getState().toString());
                ...
                case LOOKING: //if the sender is also looking
                    ...
                    if (supersedesCurrentVote(proposedLeaderID,proposedPeerEpoch)){
                       ...
                       //  incomingMessages.clear();
                        ...
                        logger.info(myPeerServer.getServerId()+" changed id from     " + lastID + "   to " + proposedLeader);
                        sendNotifications(); // tell all my peers?
                    }
    ...
    private void sendNotifications() {  // method created by me!
        // task:  send initial notifications to other peers to get things started
    ...
    protected boolean supersedesCurrentVote(long newId, long newEpoch) {
       // logger.info(" SUPERSEDES? " + this.proposedLeader + "   " + newId );
    ...
    protected void initializeLogging() {
        ...
        Logger logger = Logger.getLogger("LeaderElectionLogger");

        FileHandler fh = null;
        try {
            fh = new FileHandler("./log/LogFile_LeaderElection" + ".log");} catch (IOException e) {
            e.printStackTrace();
        }
